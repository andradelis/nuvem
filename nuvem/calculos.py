...
def convolucao(chuvas: List[float], vazoes: pd.DataFrame) -> pd.DataFrame:
    ...
    colunas = [
        pd.Series(data=np.nan, index=vazoes.index, name=_vazao) for _vazao in _vazoes
    ]
    ...
    for ix_vazao, vazao in enumerate(secao_vazoes.columns):
        for ix_chuva, chuva in enumerate(chuva_efetiva):
            try:
                secao_vazoes.iloc[inicio + ix_chuva, ix_vazao] = (
                    chuva_efetiva[ix_chuva] * vazao
                )
            except IndexError:
...
def curva_chave(
    cotas: pd.Series, vazoes: pd.Series, grau: int = 2
